...
string BattleField :: StrategicMove(int turn){
	int i, j, k;
	int piece , srow, scol, drow, dcol;
	int moves[100][4], ind;
	string myMove = "     ";
	piece = -1;
	
	if(killedPieces[0][0] == 1 && killedPieces[0][1] == 2 && killedPieces[0][2] == 3 && 
	killedPieces[0][3] == 4 && killedPieces[0][4] == 5 && killedPieces[0][5] == 1)
		return "MERCY";
	...
	while(i >= 0 && i < 7){
		for(j = 0; j < 7; j++){			
			if(board[i][j] > 0 && board[i][j] != 1 && board[i][j] != F && board[i][j] != B){
				
				if((i - 1) >= 0 && board[i - 1][j] != W && board[i - 1][j] <= 0){
					moves[ind][0] = i;
					moves[ind][1] = j;
					moves[ind][2] = i-1;
					moves[ind][3] = j;
					ind++;
					if((board[i - 1][j] < -10 && CanKill(board[i][j], fabs(board[i - 1][j]))  || board[i - 1][j] >= -10) 
					&& piece < fabs(board[i - 1][j])){
						piece = fabs(board[i - 1][j]);
						srow = i;
						scol = j;
						drow = i - 1;
				...
				if((j + 1) < 7 && board[i][j + 1] != W && board[i][j + 1] <= 0){
					moves[ind][0] = i;
					moves[ind][1] = j;
					moves[ind][2] = i;
					moves[ind][3] = j+1;
					ind++;
					if((board[i][j + 1] < -10 && CanKill(board[i][j], fabs(board[i][j + 1]))  || board[i][j + 1] >= -10) 
					&& piece < fabs(board[i][j + 1])){
						piece = fabs(board[i][j + 1]);
						srow = i;
						scol = j;
						drow = i;
				...
				if((j - 1) >= 0 && board[i][j - 1] != W && board[i][j - 1] <= 0){
					moves[ind][0] = i;
					moves[ind][1] = j;
					moves[ind][2] = i;
					moves[ind][3] = j-1;
				...
				if((i + 1) < 7 && board[i + 1][j] != W && board[i + 1][j] <= 0){
					moves[ind][0] = i;
					moves[ind][1] = j;
					moves[ind][2] = i+1;
					moves[ind][3] = j;
	...
	myMove[4] = dcol + '1';*/
	int r = rand()%ind;
	myMove[0] = moves[r][0] + 'A';
	myMove[1] = moves[r][1] + '1';
	myMove[2] = '-';
	myMove[3] = moves[r][2] + 'A';
	myMove[4] = moves[r][3] + '1';
...
int main(){
	...
	string initConfig = "CCBSLGLBBRRBMMRRFLMMR";		//---Your Initial Pieces Position String
	int turn = RED;
	BattleField myGame;
	int f = open("SPInput",O_RDWR);
	char str[100];
	srand(time(0));
	...
	write(f, (char *)opponentMoveLine1.c_str(), 5);
	write(f, "\n", 1);
	write(f, (char *)opponentMoveLine2.c_str(), 5);
	write(f, "\n", 1);
